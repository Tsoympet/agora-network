...
- **Three native PoW marks (one per layer):** TLT = L1 UTXO (RandomX); OVL = L2 native (`sha256_leading_zero`); DRC = L3 native (`sha256_leading_zero`). OVL/DRC are not L1 UTXO asset ids
