...
1. **Validate every transaction fully**, independent of conflict outcome (structure, fingerprint-bound signature, UTXO value conservation, coinbase shape).
2. **Resolve conflicts by blue order** — first accepted exact `tx_id` wins; first accepted spend of an outpoint wins. Later duplicates / conflicts are rejected even when structurally valid.
3. **Emit a deterministic `AcceptanceBitmap`** aligned to `block.transactions` indices.
4. **Sum fees only from accepted non-coinbase transactions**.
5. **Accept coinbase iff** `sum(outputs) == subsidy + accepted_fees` (genesis uses premine as subsidy).
